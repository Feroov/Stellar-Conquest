...
public class XenosphereSky extends DimensionSpecialEffects {
    ...
    public boolean renderSky(ClientLevel level, int ticks, float partialTick, PoseStack poseStack, Camera camera, Matrix4f projectionMatrix, boolean isFoggy, Runnable setupFog) {
        ...
        if(!isFoggy) {
            ...
            if(fogtype != FogType.POWDER_SNOW && fogtype != FogType.LAVA && !StarrySky.doesMobEffectBlockSky(camera)) {
                ...
                float f19 = 50.0F;
                float x2 = -650.0F;  // Keep x coordinate close to zero
                float y2 = 525.0F;  // Positive y coordinate to position the planet above the horizon
                float z2 = 100.0F;  // Keep z coordinate close to zero
